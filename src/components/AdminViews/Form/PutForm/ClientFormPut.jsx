...
export const ClientFormPut = ({ state, setState }) => {
  ...
  return (
    <div className={` ${putState.open ? 'flex' : 'hidden'} w-screen h-screen absolute top-0 left-0 z-40 flex-col items-center justify-center bg-black/[0.5] backdrop-blur-sm`}>
      ...
      <div className="bg-light-backg text-light-secondary_op flex flex-col relative p-10 rounded-3xl h-[80%] w-[60%] min-w-[10rem] max-w-[30rem] boxshadow">
        <button onClick={() => { setPutState({ ...putState, open: false }) }} className="bg-light-primary rounded-xl absolute top-5 right-5 p-2 text-light-secondary hover:scale-[1.02] active:scale-[0.97] transition-all hover:scale-[0.99]">
